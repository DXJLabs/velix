...
| File | Role |
| --- | --- |
| `veil_channel_helper.cairo` | Stores encrypted VEIL timeline event references and exposes a `privacy_invoke`-compatible helper entrypoint. |
| `veil_escrow.cairo` | Stores escrow workflow state for buyer/seller negotiation flows. |
| `escrow_interfaces.cairo` | Defines `IVeilEscrow` and a future `ISettlementAdapter` interface. |
| `escrow_types.cairo` | Defines `EscrowStatus` and the `Escrow` storage struct. |
| `escrow_events.cairo` | Defines escrow lifecycle events. |
| `escrow_validation.cairo` | Contains escrow access-control and state-transition assertions. |
