...
impl ResourceBoundsMap {
    pub fn final_apply_actions_for_validation() -> Self {
        Self {
            l1_gas: ResourceBounds::new(
                1_000_000,
                500_000_000_000_000,
            ),
            l2_gas: ResourceBounds::new(
                200_000_000,
                100_000_000_000,
            ),
            l1_data_gas: ResourceBounds::new(
                10_000,
                5_000_000_000_000,
            ),
        }
...
pub fn parse_felt(value: &str) -> Result<Felt> {
    if value.starts_with("0x") || value.starts_with("0X") {
        Felt::from_hex(value)
            .map_err(|e| anyhow!("invalid hex felt {value}: {e}"))
    } else {
        Felt::from_dec_str(value)
            .map_err(|e| anyhow!("invalid decimal felt {value}: {e}"))
    }
}
...
pub fn selector(name: &str) -> Felt {
    get_selector_from_name(name)
        .expect("selector name is valid ascii")
}
...
pub fn validate_privacy_private_key(
    scalar: &Felt,
) -> Result<()> {
    ...
    if scalar >= &half_order {
        bail!(
            "privacy private key must be canonical, \
             lower than Stark curve order / 2"
        );
    }
...
pub fn encode_set_viewing_key_client_actions(
    random: &Felt,
) -> Result<Vec<Felt>> {
    if *random == Felt::ZERO {
...
pub fn encode_compile_actions_calldata(
    ...
    out.extend(
        encode_set_viewing_key_client_actions(random)?,
    );

    Ok(out)
}

pub fn encode_account_call_array(
    pool: &Felt,
    compile_calldata: &[Felt],
) -> Vec<Felt> {
    let mut out = vec![
        Felt::ONE,
        *pool,
...
pub fn build_private_invoke(
    ...
    let compile_calldata =
        encode_compile_actions_calldata(
            user_address,
            user_private_key,
            random,
        )?;
...
pub fn redact_private_tx(
    ...
    RedactedPrivateTx {
        ...
        calldata_shape:
            "Call[1] -> privacy_pool.compile_actions(\
             user_addr, <redacted user_private_key>, \
             [SetViewingKey(random)])"
                .to_string(),
...
fn concat_resource(
    ...
    bytes[8..16].copy_from_slice(
        &bounds.max_amount.to_be_bytes(),
    );
    bytes[16..32].copy_from_slice(
        &bounds.max_price_per_unit.to_be_bytes(),
    );

    Felt::from_bytes_be(&bytes)
}

fn tip_resource_bounds_hash(
    bounds: &ResourceBoundsMap,
    tip: u64,
) -> Felt {
...
pub fn invoke_v3_hash(
    tx: &InvokeV3,
    proof_facts_override: Option<&[String]>,
) -> Result<Felt> {
    let bounds =
        rpc_bounds_to_internal(&tx.resource_bounds)?;
    ...
    let paymaster =
        hex_to_felts(&tx.paymaster_data)?;

    let account_deployment =
        hex_to_felts(&tx.account_deployment_data)?;

    let calldata =
        hex_to_felts(&tx.calldata)?;
...
fn parse_u64_hex(value: &str) -> Result<u64> {
    ...
    u64::from_str_radix(trimmed, 16)
        .map_err(|e| {
            anyhow!(
                "invalid u64 hex {value}: {e}"
            )
        })
}

fn rpc_bounds_to_internal(
    value: &RpcResourceBoundsMap,
) -> Result<ResourceBoundsMap> {
    Ok(ResourceBoundsMap {
        l1_gas:
            rpc_bound_to_internal(&value.l1_gas)?,
        l2_gas:
            rpc_bound_to_internal(&value.l2_gas)?,
        l1_data_gas:
            rpc_bound_to_internal(
                &value.l1_data_gas,
            )?,
    })
}

fn rpc_bound_to_internal(
    value: &RpcResourceBounds,
) -> Result<ResourceBounds> {
    Ok(ResourceBounds {
        max_amount:
            parse_u64_hex(&value.max_amount)?,
        max_price_per_unit:
            parse_u128_hex(
                &value.max_price_per_unit,
            )?,
...
pub fn sign_hash(
    ...
    let signature =
        sign(account_private_key, hash, &k)
            .map_err(|e| {
                anyhow!("sign failed: {e}")
            })?;
...
pub fn proof_facts_info(
    proof_facts: &[String],
) -> Result<ProofFactsInfo> {
    let felts = hex_to_felts(proof_facts)?;

    if felts.len() < 8 {
        bail!(
            "proof_facts too short: {}",
            felts.len(),
        );
    }

    let message_count =
        felt_to_usize(&felts[7]).ok();

    let hashes = match message_count {
        Some(count)
            if felts.len() >= 8 + count =>
        {
            felts[8..8 + count]
                .iter()
                .map(felt_hex)
    ...
    Ok(ProofFactsInfo {
        ...
        virtual_program_hash:
            Some(felt_hex(&felts[2])),
        os_output_version:
            Some(felt_hex(&felts[3])),
        base_block_number:
            Some(felt_hex(&felts[4])),
        base_block_hash:
            Some(felt_hex(&felts[5])),
        config_hash:
            Some(felt_hex(&felts[6])),
        message_count,
...
pub fn assert_proof_facts_header(
    info: &ProofFactsInfo,
) -> Result<()> {
    let expected_variant =
        felt_hex(
            &parse_felt(VIRTUAL_SNOS_HEX)?,
        );

    let expected_output =
        felt_hex(
            &parse_felt(VIRTUAL_SNOS0_HEX)?,
        );

    if info.program_variant != expected_variant {
        bail!(
            "invalid program variant: {}",
            info.program_variant,
        );
    }

    if info.os_output_version.as_deref()
        != Some(expected_output.as_str())
    {
        bail!(
            "invalid OS output version: {:?}",
            info.os_output_version,
        );
    }
...
pub fn recover_server_actions_from_l2_message(
    ...
    let payload =
        hex_to_felts(&message.payload)?;

    if payload.first() != Some(pool_class_hash) {
        bail!(
            "L2->L1 payload class hash does not \
             match deployed pool class hash"
        );
    }

    // Payload is:
    // [privacy_pool_class_hash,
    //  serialized Span<ServerAction>].
    //
    // The returned value already includes the
    // Cairo Span length felt.
...
pub fn build_final_apply_actions_tx(
    ...
    // `server_actions` is already the full ABI
    // calldata for:
    //
    // apply_actions(actions: Span<ServerAction>)
    //
    ...
    Ok(InvokeV3 {
        ...
        version: "0x3".to_string(),
        sender_address:
            felt_hex(submitter_address),
        calldata:
            felts_to_hex(&calldata),
        signature: vec![],
        nonce:
            felt_hex(nonce),
        resource_bounds:
            ResourceBoundsMap::
                final_apply_actions_for_validation()
                .into(),
        tip: "0x0".to_string(),
        paymaster_data: vec![],
        account_deployment_data: vec![],
...
pub fn sign_final_tx(
    ...
    if hash_without == final_hash
        && !tx.proof_facts.is_empty()
    {
        bail!(
            "final hash unexpectedly did not \
             change after proof_facts"
        );
    }
...
pub fn final_summary(
    ...
    FinalTxSummary {
        ...
        proof_facts_len:
            tx.proof_facts.len(),
        hash_without_proof_facts:
            felt_hex(hash_without),
        final_hash:
            felt_hex(final_hash),
        signature_len:
            tx.signature.len(),
...
pub fn ensure_localhost(url: &str) -> Result<()> {
    ...
    if !matches!(
        ...
        bail!(
            "prover URL must be localhost, got {host}"
        );
    }
...
>(
    ...
    let response: JsonRpcResponse<T> =
        client
            ...
            .with_context(|| {
                format!(
                    "RPC response decode failed: {method}"
                )
            })?;
...
pub fn prove_transaction(
    ...
    {
        bail!(
            "transaction prover input must have \
             empty proof and proof_facts"
        );
    }
...
pub fn compute_contract_class_hash(
    path: &Path,
) -> Result<Felt> {
    let raw =
    ...
    match artifact {
        ContractArtifact::SierraClass(class) => {
            class
                .class_hash()
                .map_err(Into::into)
        }

        ContractArtifact::CompiledClass(class) => {
            class
                .class_hash()
                .map_err(Into::into)
        }

        ContractArtifact::LegacyClass(class) => {
            class
                .class_hash()
                .map_err(Into::into)
        }
...
pub fn artifact_json<T: Serialize>(
    value: &T,
) -> Result<String> {
    Ok(
            ...
            .map_err(|_| {
                anyhow!(
                    "{name} is required and must \
                     not be passed as a CLI argument"
                )
            })?;

    let felt =
        parse_felt(value.trim())?;
...
mod tests {
    ...
    fn set_viewing_key_encoding_matches_cairo_serde_shape() {
        ...
        let encoded =
            encode_set_viewing_key_client_actions(
                &random,
            )
            .unwrap();
    ...
    fn private_invoke_shape_has_empty_proof_fields() {
        let pool =
            parse_felt(PRIVACY_POOL_ADDRESS)
                .unwrap();

        let user =
            parse_felt("0x123").unwrap();

        let key =
            parse_felt("0x456").unwrap();

        let random =
            parse_felt("0x789").unwrap();

        let tx =
            build_private_invoke(
                &pool,
                &user,
                &key,
                &random,
    ...
    fn proof_facts_header_deserializes() {
        ...
        assert_proof_facts_header(&info)
            .unwrap();
    }
    ...
    fn l2_message_recovery_keeps_serialized_server_action_span() {
        ...
        let actions =
            recover_server_actions_from_l2_message(
                &class_hash,
                &message,
            )
            .unwrap();
        ...
        assert_eq!(
            expected,
            poseidon_hash_many(&raw_message),
        );
    }
    ...
    #[test]
    fn rpc_call_error_display_includes_data() {
        let error = RpcCallError {
            code: Some(-32603),
            message:
